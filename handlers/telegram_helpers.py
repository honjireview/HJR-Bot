...
def _extract_message_content(message) -> Dict[str, Any]:
    """Извлекает основной контент из сообщения."""
    if not message:
        return {}
    content = {}
    ...
    if poll:
        ...
        content["poll"] = {
            ...
            # ИСПРАВЛЕНО: Добавляем флаг, закрыт ли опрос
            "is_closed": getattr(poll, "is_closed", True) # Считаем закрытым по умолчанию, если поле отсутствует
        }
        log.debug(f"[_extract] Extracted poll: {content['poll']['question']}, is_closed: {content['poll']['is_closed']}")
        return content
    ...
    try:
        ...
        if content:
            ...
            if temp_message.is_topic_message:
                thread_id = temp_message.message_thread_id
...
def validate_appeal_link(bot, url: str, user_chat_id: int) -> (bool, Union[str, Dict]):
    ...
    # ИСПРАВЛЕНО: Добавлена проверка на открытый опрос
    if content.get("type") == "poll" and not content.get("poll", {}).get("is_closed"):
        log.warning("[VALIDATOR] FAILED: Poll is still open.")
        return False, "Нельзя подать апелляцию на опрос, который еще не завершен. Пожалуйста, дождитесь его закрытия."
...
def get_discussion_context(bot, chat_id, message_id, thread_id=None, limit=3):
    """
    Функция-заглушка. Сбор контекста временно отключен из-за ограничений библиотеки.
    """
    log.warning(f"Сбор контекста для {chat_id}/{message_id} пропущен: функция get_chat_history не поддерживается библиотекой pyTelegramBotAPI.")
    return "Контекст обсуждения временно не доступен."