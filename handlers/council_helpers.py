...
def request_counter_arguments(bot, case_id: int):
    ...
    # Формируем полный, информативный текст запроса
    request_text = (
        ...
        f"3. *Доп. контекст:* {q3}\n\n"
        f"---"
        f"\nРедакторам предлагается предоставить свои контраргументы. "
        f"Срок: 24 часа.\n\n"
        f"👇 *Чтобы ответить, скопируйте и отправьте команду ниже:* 👇\n"
        f"`/reply {case_id}`"
    )
